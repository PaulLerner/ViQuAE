...
class ICADataModule(QuestionAnsweringDataModule): 
    def __init__(self, *args, text_key, image_key, **kwargs):
        super().__init__(*args, **kwargs)
        self.text_key = text_key
        self.padding_passage = [{}]
        self.image_key = image_key
        self.add_image = self.add_image_features
        if self.image_formatter.precomputed is not None:
            warnings.warn("Not using self.image_formatter")
    ...
    def add_image_features(self, passages):
        ...
        batch = {'index': [], self.text_key: []}
        for passage in passages:
            batch['index'].append(passage['index'])
            batch[self.text_key].append(passage[self.text_key])
        subset = self.image_kb.select(batch['index'])
        for feature in features:
            batch.setdefault(feature, subset[feature])
        # dict of list to list of dict
    ...
    def collate_fn(self, items):     
        ...
        for i, item in enumerate(items):
            ...
            questions.append(item[self.text_key])
            relevant_passages.extend(relevant_passage)
            irrelevant_passages.extend(irrelevant_passage)

        questions_text = torch.stack(questions)
        questions_image = torch.stack([item[self.image_key] for item in items])
        
        all_passages = relevant_passages + irrelevant_passages
        all_passages_text = torch.zeros(len(all_passages), questions_text.shape[1])
        all_passages_image_features = torch.zeros(len(all_passages), questions_image.shape[1])
        # 0=masked, 1=not masked
        all_passages_image_attention_mask = torch.zeros(len(all_passages))
        for i, passage in enumerate(all_passages):
            if self.text_key in passage:
                all_passages_text[i] = passage[self.text_key]
            if self.image_key in passage:
                all_passages_image_features[i] = passage[self.image_key]
                all_passages_image_attention_mask[i] = 1.0
                                                        
        # wrap it up
        question_inputs = {"embeddings": questions_text, "image_context": questions_image.unsqueeze(1)}
        context_inputs = {
            "embeddings": all_passages_text, 
            "image_context": all_passages_image_features.unsqueeze(1),
            "image_attention_mask": all_passages_image_attention_mask.unsqueeze(1)
        }
